...
public class CameraOrbit : MonoBehaviour
{
    ...
    {
        float heigth = Mathf.Tan(camera.fieldOfView * Mathf.Deg2Rad / 2) * camera.nearClipPlane;
        float width = heigth * camera.aspect;

        nearPlaneSize = new Vector2(width, heigth);
    }
    ...
    void Update()
    {
        ...
        if (hor != 0)
        {
            angle.x += hor * Mathf.Deg2Rad * sensitive.x;
        }
        ...
        if (ver != 0)
        {
            angle.y += ver * Mathf.Deg2Rad * sensitive.y;
